...
if(isset($_REQUEST['p_letras'])){
	...
	$t_prepagada = buscar_prepagada ($conn, $v_letras);
	dbdisconn ($conn);
	...
	echo ("{\"results\": [");
	$arr = array();
	if (is_array($t_prepagada)) {
		foreach($t_prepagada as $dato) {
			$arr[] = "{\"id\": \"".$dato['id_prepagada']."\", \"value\": \"".utf8_encode($dato['nombre'])."\", \"info\": \"\"}";
		}
		echo implode(", ", $arr);
	}
	echo ("]}");
