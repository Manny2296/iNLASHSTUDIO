...
include ($path."/lib/parametros_dml.php");

$conn  = dbconn ($db_host, $db_name, $db_user, $db_pwd);
$skin  = obtener_skin ($conn);

if (isset($_SESSION['id_perfil'])) {
	if ( validar_permisos ($conn, 'exec_upd_config.php') ) {
		$t_params = obtener_parametros($conn);
		foreach ($t_params as $dato){
			upd_parametro($conn, $dato['id_parametro'], $_POST['para_'.$dato['id_parametro']]);
		}
?>
...
<title>.:: iNLASH & Co - Configuraci&oacute;n del sistema ::.</title>
<!-- InstanceEndEditable -->
...
<body>
  <div id="contendor">
	 ...
	 <?php mensaje(1, 'Los par&aacute;metros fueron actualizados correctamente', 'sysconfig_frm.php', '_self'); ?>
	 <!-- InstanceEndEditable -->
     </div>
  </div>
</body>
