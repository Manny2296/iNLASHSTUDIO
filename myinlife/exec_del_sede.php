...
if (isset($_SESSION['id_perfil'])) {	
	if ( validar_permisos ($conn, 'exec_del_programacion_adm.php') ) {
		...
		$t_valores = array('', '');
		$v_result = del_sede ($conn, $v_id_sede);
?>
...
<title>.:: MY INLIFE STUDIO - Eliminaci&oacute;n de programaci&oacute;n ::.</title>
<!-- InstanceEndEditable -->
...
<body>
  <div id="contendor">
	 ...
	 <?php 
    if($v_result) {
  		mensaje_form(1, 'La Sede fue eliminada correctamente', 'sedes_lst.php', $t_campos, $t_valores); 
	} else {
		mensaje_form(2, 'La Sede no pudo ser eliminada debido a q tiene facturas o clientes en el sistema, se ha marcado como inactiva', 'sedes_lst.php', $t_campos, $t_valores); 
	}
	?>
	 <!-- InstanceEndEditable -->
     </div>
