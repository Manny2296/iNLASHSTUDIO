...
if (isset($_SESSION['id_perfil'])) {	
	if ( validar_permisos ($conn, 'exec_del_comentario.php') ) {
		$v_id_usuario = $_POST['p_id_usuario'];
		$v_fecha = $_POST['p_fecha'];
		
		$v_result = del_observacion($conn, $v_id_usuario, $v_fecha);
?>
...
<title>.:: MY INLIFE STUDIO - Eliminaci&oacute;n de comentarios de la ficha antropom&eacute;trica ::.</title>
<!-- InstanceEndEditable -->
...
<body>
  ...
  <?php 
    if($v_result) {
  		mensaje(1, 'El comentario fue eliminado correctamente', 'cliente_ficha_frm.php?p_id_usuario='.$v_id_usuario, '_self'); 
	} else {
		mensaje(2, 'El comentario no pudo ser eliminado', 'cliente_ficha_frm.php?p_id_usuario='.$v_id_usuario, '_self'); 
	}
	?>
  <!-- InstanceEndEditable -->
  </div>
