	...
	var imgDir = "lib/img/"	// directory for images ... e.g. var imgDir="/img/"
	
	var gotoString = "Ir al mes actual"
	var todayString = "Hoy es"
	var weekString = "Sem"
	var scrollLeftMessage = "Oprima para ir al mes anterior. Sostenga el boton del mouse para retroceder automáticaticamente."
	var scrollRightMessage = "Oprima para ir al mes siguiente. Sostenga el boton del mouse para avanzar automáticaticamente."
	var selectMonthMessage = "Clic para seleccionar el mes."
	...
	var	crossobj, crossMonthObj, crossYearObj, monthSelected, yearSelected, dateSelected, omonthSelected, oyearSelected, odateSelected, monthConstructed, yearConstructed, intervalID1, intervalID2, timeoutID1, timeoutID2, ctlToPlaceValue, ctlNow, dateFormat, nStartingYear, pc_callback_fn
...
//---------------------------------------------TRADUCCION EN ESPAÑOL-----------------------------------------------------------------------------
	...
	function init()	{
		...
		{	
			...
			if (!ie) { yearNow = today.getYear()+1900 }
	...
	function closeCalendar() {
		...
		if (pc_callback_fn != "undefined" && typeof(pc_callback_fn) == "function") {
			    pc_callback_fn();
		}
	}
	...
	function ChangeYear(e){
		...
		if( keynum >= 48 && keynum <=57 ){
			numcheck = document.getElementById("year").value+keychar;
			/*alert(document.getElementById("year").value+keychar);*/
		}else if( keynum == 8 ){
			/*alert( document.getElementById("year").value.substr(0, document.getElementById("year").value.length -1));	*/
			numcheck = document.getElementById("year").value.substr(0, document.getElementById("year").value.length -1);
		}else if( keynum == 13){
		    numcheck = document.getElementById("year").value+keychar;
		}
		nYear = numcheck;
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			/*CODIGO PARA GENERACION DE AÑOS EN PULLDOWN*/
			...
			for	(i=yearNow+2; i>=(yearNow-90); i--) {		
				sName =	i;
				if (i==yearSelected){
					sName =	"<B>" +	sName +	"</B>"
				}
				...
				sHTML += "<td id='y" + sName + "' onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='this.style.backgroundColor=\"\"' style='cursor:pointer' onclick='selectYear("+i+");event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td>"
			} 
			
			sName = yearSelected;
			//sHTML += "<td id='" +sName+ "'onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='this.style.backgroundColor=\"\"' style='cursor:pointer'>	<input id='year' MAXLENGTH='4' value='" +sName+ "' name='date' type='text' size='5' onclick='event.cancelBubble=true' onkeypress='ChangeYear(event)'></td>"
			sHTML += ""
...
//------------------------------------TERMINAR LA FUNCION EN 0 PARA QUE INICIE EN EL DIA ACTUAL EN LA SIGUIENTE LINEA----------------------------------------------
	...
	function popUpCalendar(ctl,	ctl2, callbackfn) {
		var 	format = "dd-mm-yyyy";
		/*Coordenadas respecto al campo*/
		//LONGITUD DEL CALENDARIO
		var	leftpos=10
		var	toppos=5
		//LONGITUD DEL CALENDARIO
		var longitud = ((showWeekNumber==1)?250:220);	
		
		var winx = screen.width - 185;
...
//------------------------------------TERMINAR LA FUNCION EN 0 PARA QUE INICIE EN EL DIA ACTUAL EN LA SIGUIENTE LINEA----------------------------------------------
				...
				ini = ctl.offsetLeft + leftpos;
				fin = longitud + (ctl.offsetLeft + leftpos);
				
				if( fin > winx ){
				    diferencia = fin - winx;
					crossobj.left = ini -  (diferencia+35) ;
				}
